...
from utils import (
    ...
    assert_true, deploy_contract_with_prepayment, print_section, results
)
...
def test_storage_set_and_get(ctx):
    ...
    contract = deploy_contract_with_prepayment(ctx, STORAGE_TEST_SOL, "StorageTest")
...
def test_arithmetic_add(ctx):
    """Test ADD opcode."""
    contract = deploy_contract_with_prepayment(ctx, ARITHMETIC_TEST_SOL, "ArithmeticTest")
...
def test_comparison_eq(ctx):
    """Test EQ opcode."""
    contract = deploy_contract_with_prepayment(ctx, COMPARISON_TEST_SOL, "ComparisonTest")
...
def test_log_ops(ctx):
    """Test LOG opcodes."""
    contract = deploy_contract_with_prepayment(ctx, LOG_TEST_SOL, "LogTest")
...
def test_selfbalance(ctx):
    ...
    contract = deploy_contract_with_prepayment(ctx, SELFBALANCE_TEST_SOL, "SelfBalanceTest", amount=1000000)
